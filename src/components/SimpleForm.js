...
function SimpleForm() {
  ...
  const schema = {
    name: Joi.string().min(1).max(20).required(), //what is the rule?
    email: Joi.string().email().required(),
    //Mini exercise: age should be a number from 1 to 100 and is required.
  ...
  */
  ...
  const validate = (event) => {
    ...
    const objToCompare = {[name] : value}; //use the [] to get the value of the name. because if we didn't, the key will be always "name"
    //Recall when you are creating objects it follows the format {key : value}, typically, the key is a string.
    //If I don't put [], if I type in the email field, the objToCompare will be {name: terence.gaffud@skillsunion} or if I type in the age field, the objToCompare will be {name : 35}
    ...
    //we get the errors and return if there are any.
    const {error} = result;
